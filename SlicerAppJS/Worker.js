 ...
 * Time: 12:18 PM
 */
const GCODE = require("./gCodeReader.js");
...
var sendMultiLayerToParent = function (layerNum, z, progress) {
  ...
  //   console.log("sendMultiLayerToParent: ", message);

  GCODE.processMultiLayerFromWorker(message);
};
...
var sendAnalyzeDone = function () {
  ...
  GCODE.processAnalyzeModelDone(message);
  // GCODE.passDataToRenderer();
  // printModelInfo();
  // printLayerInfo(0);
};
  ...
  for (i = 0; i < model.length; i++) {
    ...
    for (j = 0; j < cmds.length; j++) {
      ...
      if (
        ...
        max.x =
          parseFloat(max.x) > parseFloat(cmds[j].x) ? parseFloat(max.x) : parseFloat(cmds[j].x);
        max.x =
          parseFloat(max.x) > parseFloat(cmds[j].prevX)
            ? parseFloat(max.x)
            : parseFloat(cmds[j].prevX);
        min.x =
          parseFloat(min.x) < parseFloat(cmds[j].x) ? parseFloat(min.x) : parseFloat(cmds[j].x);
        min.x =
      ...
      if (
        ...
        max.y =
          parseFloat(max.y) > parseFloat(cmds[j].y) ? parseFloat(max.y) : parseFloat(cmds[j].y);
        max.y =
          parseFloat(max.y) > parseFloat(cmds[j].prevY)
            ? parseFloat(max.y)
            : parseFloat(cmds[j].prevY);
        min.y =
          parseFloat(min.y) < parseFloat(cmds[j].y) ? parseFloat(min.y) : parseFloat(cmds[j].y);
        min.y =
  ...
  for (var i = 0; i < gcode.length; i++) {
    ...
    if (reg.test(gcode[i])) {
      ...
      for (j = 0; j < args.length; j++) {
        // console.log("i: ", i, " j: ", j, " args: ", args[j]);
        //                        console.log(args);
        //                        if(!args[j])continue;
...
// module.exports = worker.work;
module.exports.parseGCode = parseGCode;
