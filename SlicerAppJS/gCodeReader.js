...
var GCODE = {};                                  //gcode object used to be able to call gcode reading related functions.

GCODE.gCodeReader = (function () {               
  var gcode, lines;                              //gcode date to be considered
  var z_heights = {};
  var model = [];
  var max = {
    x: undefined,
  ...
  var prepareGCode = function () {                                                        //filters out irrelavent gcode commands
    if (!lines) return;
    gcode = [];
    var i;
  ...
  var sortLayers = function () {                                                          //filter out irrelavent layer data information
    var sortedZ = [];
    var tmpModel = [];
    //        var cnt = 0;
  ...
  var getParamsFromKISSlicer = function (gcode) {                                          //Each slicer assumes different paramter values which are 
                                                                                           //apparent in the gcode files produced by them; need to be considered when reading
  ...
  var detectSlicer = function (gcode) {                                                     //detects the slicer from the input gcode file
  ...
  return {
    ...
    processLayerFromWorker: function (msg) {                                                //the worker provdes math calculations to obtain model info results
      //            var cmds = msg.cmds;
    ...
    processAnalyzeModelDone: function (msg) {
      ...
      totalWeight =
        (((((density * 3.141 * gCodeOptions["filamentDia"]) / 10) * gCodeOptions["filamentDia"]) /
          10 /
          4) *
          totalFilament) /
        10;
    ...
    getModelInfo: function () {                                                        //returns model info
      return {
        min: min,
        max: max,
        modelSize: modelSize,
...
module.exports = GCODE.gCodeReader;
